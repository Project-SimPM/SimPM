...
def _render_table_with_preview(title: str, df: pd.DataFrame, key_prefix: str) -> None:
    ...
    icon_html = (
        f"<img src=\"data:image/png;base64,{download_icon_b64}\" id=\"download-icon\" "
        f"style=\"position: absolute; top: 4px; right: 4px; width: 24px; height: 24px; cursor: pointer;\" "
        f"onclick=\"window.location.href='data:text/csv;base64,{csv_b64}'\" alt=\"Download CSV\" />"
        if download_icon_b64
        else ""
    )
    table_html = (
        f"<div class=\"simpm-table-wrapper\" style=\"position: relative;\">{icon_html}{df.head(5).to_html(index=False)}</div>"
    )
