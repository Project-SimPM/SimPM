...
def _render_queue_waiting_log(df: pd.DataFrame, key_prefix: str) -> None:
    """Render queue log with waiting duration plots for resources."""
    ...
    tab_table, tab_hist, tab_stats = st.tabs(["Table", "Histogram", "Statistics"])

    with tab_table:
        _render_table_preview("Queue log", df, key_prefix)

    if "waiting_duration" not in df.columns or df["waiting_duration"].dropna().empty:
        for tab in (tab_hist, tab_stats):
            with tab:
                st.info("No waiting duration data available to plot.")
        return
...
def _render_resource_status(df: pd.DataFrame, key_prefix: str) -> None:
    """Render resource status log with time series and distribution plots."""
    ...
    tab_series, tab_hist, tab_stats, tab_table = st.tabs(
        ["Time series", "Histogram", "Statistics", "Table"]
    )

    long_df = df[[time_axis] + metric_cols].melt(id_vars=time_axis, value_vars=metric_cols, var_name="metric", value_name="value")

    with tab_series:
        fig = px.line(long_df, x=time_axis, y="value", color="metric", labels={"value": "Value", time_axis: time_axis})
        fig.update_traces(line_shape="hv", selector=None)
        st.plotly_chart(fig, width="stretch")

    with tab_hist:
        metric = st.selectbox("Select metric", metric_cols, key=f"{key_prefix}-status-metric")
        fig = px.histogram(df, x=metric, nbins=min(30, max(5, len(df))))
        fig.update_traces(marker_color="#5bbd89")
        st.plotly_chart(fig, width="stretch")

    with tab_stats:
        metric = st.selectbox(
            "Metric for statistics",
            metric_cols,
            key=f"{key_prefix}-status-metric-stats",
        )
        stats_df = _basic_statistics(pd.to_numeric(df[metric], errors="coerce"))
        st.dataframe(stats_df, width="stretch")
